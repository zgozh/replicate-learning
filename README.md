...
> 不是「源码阅读 Prompt」，也不是「AI 自动写文档」——它交付的是**三本教材 + 一套工程训练日志**，并且每一条结论都要能被机械闸门复检。

[![判据版本](https://img.shields.io/badge/判据-v2.23-blue)](skills/replicate-learning/SKILL.md)
[![自检](https://img.shields.io/badge/selfcheck-76%20项%200%20失败-brightgreen)](skills/replicate-learning/scripts/skill_selfcheck.py)
...
三本教材**同步推进、互相回链**，工程日志**独立于被学习项目**存放（不污染目标仓库）。
...
| 常见做法 | 本 Skill |
|---|---|
| 让 AI "讲一下这个项目"，产出一段说明 | 产出三本教材 + 工程日志，每份有固定节序与必含项 |
| 讲解质量靠"模型自觉" | **12 个脚本 + 76 项自检 + 七项闸门**机械复检，不过不让 commit |
...
| 规则改了没人知道 | 判据有**版本号**（当前 v2.23）+ **变更登记表** + **回归集比对**，改动必须走三件套 |
...
python scripts/skill_selfcheck.py             # 期望：检查项 76，失败 0 → PASS ✅
...
3. **工程日志目录**（默认目标项目同级 `.replicate-learning-log/<项目名>/`）
...
NOTES/
  ...
  教学讲解/第二册-实验与机制验证/       ← 为什么、改了会怎样
  教学讲解/第三册-架构挑战与独立重构/   ← 不照抄我会怎么做
```
...
| **判据版本** | `SKILL.md` §6.6 | 当前 **v2.23**；任何判据变更必须走三件套 |
| **技能自检** | `scripts/skill_selfcheck.py` | 76 项，对账 SSOT ↔ SKILL ↔ 模板 ↔ gate，防"规则丢失" |
...
| `skill_selfcheck.py` | 技能文档一致性自检（SSOT ↔ SKILL ↔ 模板 ↔ gate 对账，76 项） |
...
| `safe_edit.py` | **改讲解文件的唯一写入口**：围栏护栏 + 整节重写，跨围栏替换一律拒绝 |
...
**硬约束**（写在 `SKILL.md` §1，不可协商）：

1. 教材写入被学习项目 `NOTES/`；工程日志独立保存，不污染目标项目。
2. 三本书默认同步推进；用户明确要求"先要第一册"时可进单册优先模式，但**暂停必须显式落盘**（状态文件写明原话 + 预计何时补 + 能力账本记欠账），不是悄悄不做。
...
**首轮试运行的最小验收集**：四份基础地图 + 第一册一个源码批次 + 一组 Prediction/Evidence/Experiment + 一个 Architecture Challenge + 一个 Issue 八步闭环 + 一次用户 Diff Review 与最终 Accept。
...
| 维度 | V1 | V2 |
|---|---|---|
| 教材 | 第一册 | **三本书同步推进**（源码实现 / 实验验证 / 架构挑战） |
| 工程训练 | 无独立载体 | **独立工程日志** + Issue 八步协议 + 能力账本 |
| 受众 | 讲给"读者" | 同时讲给"要动手改它的人"（⑫ Vibecoding 视角 +【怎么用】/【怎么接】） |
| 判据 | 2.14 | **2.23**（含教材类/记录类文件形态判定、注释标记按语言取前缀、件段边界只认正文等） |
