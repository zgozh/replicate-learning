...
> **2026-09-20 用户决定：这条工单不再处理。** 批次3 按自己的声明判 FAIL、**保留现状**（不补 ⑥ 的逐步回放，
> 也不改声明）；它仍是内容/结构基准批，⓪d 的那条缺口如实留在闸门输出里。
> 已在 `spec/操作手册-闸门与工具.md` §23.1 与 `references/第一册质量细则.md` §6.6 登记同一结论，
> 后续会话不要把它当未决工单重开。
...
## 14. 批次49 成品缺陷与 ⑨ 改版（2026-09-20，用户通读后）

### 14.1 ⑥ 件内顺序被"骨架生成器"改坏（血证 H28 · 第二轮 §3.3 的代码）

用户通读批次49 后指出：**⑥ 四个件的三段开场全排在源码块之后**。对照批次7（正确形态）：

```text
批次7（正确）  件标题 → 本文件要解决的一个问题 → 白话开场 → 构造方式与手法 → ```java 源码 → 逐行要点表 → …
批次49（错误）  件标题 → 槽位标记 → ```java 源码 → 本文件要解决的一个问题 → 白话开场 → 构造方式与手法 → …
```

**根因在工具，不在模型**：`new_batch.plan_section()`（§3.3 新写的骨架生成）把**槽位标记 + 空围栏排在了
件标题之后**，把模板里的三段开场挤到代码块下面；模型照着骨架写，于是四个件一致地错。
模板当时是对的——是"模板的可执行版本"改了顺序（H28：**工具也是模板的下游**）。

修法：`plan_section` 件内顺序与模板**逐行一致**（三段开场 → 槽位/源码块 → 逐行要点表）；加两道机械断言——
`skill_selfcheck`「骨架件内顺序 = 模板顺序」（相对顺序断言，改模板也一起兜住）+
`test_batch_slots.test_item_body_keeps_the_template_reading_order`。

**批次49 成品已回流**：纯移动脚本把四个件的三段开场移回代码块之前——**非空行多重集逐行不变、总行数不变
（1904）**，校验不过即拒绝写盘；回流后闸门 PASS 并重新盖章（⑯ 记新正文哈希）。批次48 不受影响
（它的 ⑥ 顺序本来就对，写于新流水线之前）。

### 14.2 ⑨ 由「No-Framework 等价实现」改为「八股讲解」（判据 2.31）

用户要求："⑨ 的 No-Framework 我用不上，把这节删了，换成八股讲解——读讲解时顺手把面试考点也学了。"

**结构决定：原位替换，不删节不重编号。** ⑨ 仍占第 9 节、仍是 17 节——⑩~⑯ 的编号与全库既有锚点
（⑫ 八条 / ⑯ 自检 / ⑬.2 …）零改动。真删掉 ⑨ 再加一节会让 49 批的节编号与全部引用重排，代价远大于收益。

| 项 | 内容 |
|---|---|
| 新标题 | `## ⑨ 八股讲解（本批涉及的面试与工程常识）` |
| 内容（SSOT `S25`） | ≥6 条考点（每件至少 1 条），每条四段：**【考点】**（面试官怎么问）/**【一句话定义】**（标准答法）/**【为什么考】**（考什么能力、答错的常见形态）/**【本批落点】**（回链 `6.x` 或 `:Lnn`，**没有落点的不许写**）；段末一张「本批八股速查表」 |
| 机械判定 | 闸门新检查 **`G-KNOW`**（`KNOW_FAIL_SINCE=2.31` 版本门）：条数 / 四段字段 / 回链 / 速查表；免检词表补「八股/面试」（本节代码按教学示例免检） |
| 判定档位 | 声明判据 ≥2.31 → FAIL 档；**存量 49 批仍是 No-Framework → 报告档不追溯**（只报一句"⑨ 仍是旧版"） |
| 版本 | `GATE_VERSION` 2.30 → **2.31**，SSOT version 同步；§6.6 变更登记加一行；血证 H28 记录 ⑥ 顺序事故 |

**批次49 的 ⑨ 已按新判据重写**：9 条考点（分块策略 / overlap / 裸 substring / 归一化代价 / 向量文本 vs 展示文本 /
雪花 ID / 不可变对象 / pieces 不可撤销契约 / 怎么评测切块），每条都回链本批 `6.x` 与 `:Lnn`（37 处 `:L` 引用），
带速查表——闸门在 **FAIL 档**下 PASS。

**一个必须知道的连带影响**：`sync_gate_result --apply` 会把 ⑯ 的版本声明刷成当前版，所以**给任何旧批次重新盖章，
都会让它按 2.31 从严重判**（⑨ 必须先换成八股，否则盖章后复跑必 FAIL）。这是既有的"声明即承诺"语义
（2.26/2.27/2.28/2.29 各门同理），不是新引入的例外；回修旧批时请顺带换 ⑨（模板已换，夹具也带升级脚本）。

### 14.3 本轮验证

```text
skill_selfcheck      检查项 115，失败 0 → PASS ✅（+2：件内顺序 = 模板 / ⑨ 八股判据）
v2_selfcheck         PASS (6 contract entries)
单测                 Ran 152 tests OK（+8：test_know_section；+1：件内顺序；版本门测试改为跟随 GATE_VERSION）
e2e                  29 步期望全中（真实批次48 副本先升级 ⑨ 再走"门禁→盖章→发布"闭环）
回归                 13 份样本（批次1/3/33/40/43/47/48/49 + 5 份仓内样例）退出码/总判定/[FAIL] 行数逐字不变
```

**仍未解决**（同 §13.4）：模型写作耗时仍未采集；⑨ 换节只是把"面试考点"补进教材，不改变整批写作时长。

## 15. 第七轮：发布器补上"回修"这半条生命周期（第五种 op `update_line`）

### 15.1 触发：实现模型报的通道缺口（用户转述，2026-09-21）

> `publish_batch.py` 是首次发布通道：`idempotent_key` 一旦已出现在文件里即判"已发布过"跳过，
> op 也只有 `append/insert/replace_anchor/ensure_line` 四种 —— **回修已发布批次的既有行（例如把行内
> v2.30 改成 v2.31）没有对应 op，只能人工改**。我因此没敢动 skill，改用带断言的脚本
> （`b49_fix231_views.py`）完成。若你希望"回修"也走统一通道，建议给 `publish_batch` 加一个
> `update_line`（anchor + expect + text 覆盖既有行）之类的 op。

**决定：加，但不按"能覆盖既有行"这个最弱形态加。** 先复核缺口本身是否只有"少一个 op"这么简单——
把回修硬套进旧 op，会发现**两条路都走不通**，而且失败形态不同：

| 拿旧 op 做回修 | 实际会发生什么 |
|---|---|
| `replace_anchor`（anchor = 旧行，text = 新行） | 第一次能改对；**第二次运行 anchor 已不存在 → 「锚点命中 0 处」冲突**。即"回修"这条通道**不幂等**——重复运行会红，而"重复运行零变化"是本工具写在文档里的承诺 |
| 任意 op + `idempotent_key` | 回修对象本来就是已发布批次 → **key 必然已在文件中** → 整条 target **静默跳过**（rc=0，一个字节没动）。"成功"与"没生效"在输出上分不开 |

所以真正的缺陷不是"op 少了"，而是**幂等的判据建立在"操作痕迹"（key 在不在文件里）上**：对"首次发布"
它成立，对"回修"必然失效。缺口下游的后果也很具体——**一次性脚本又回来了**（逐处找锚点、不在技能里、
无审计、下次没人能复用它），正是 §3.5 要消灭的形态。记为**血证 H29**。

### 15.2 语义：`update_line` 是"结果状态判幂等"的一种 op

| 字段 | 含义 | 为什么必须这样 |
|---|---|---|
| `anchor` | 定位串，**回修之后仍然存在**且唯一命中 | 若用旧行当 anchor，回修一次后它就没了 → 下次运行分不开"已回修过"与"记录写错"。**这是机械护栏**：不满足时当场报「锚点回修后命中 0 处」，幂等不许只靠承诺 |
| `expect` | 该行**当前的整行原文**（整行比对，允许首尾空白差异） | 回修改的是整行；允许子串就等于允许"没读过这行就改掉它"——`replace_anchor` 的 `expect` 仍是子串语义，**故意不改它**（改它会静默改变既有记录的含义） |
| `text` | 回修后的整行（**单行**） | 多行会让"这行改没改过"无法判定；块级改动仍走 `insert_before_anchor` / `replace_anchor` |

幂等三态：`行 == text` → 跳过（已回修过）｜`行 == expect` → 改｜**其他 → 中止**（不盲改；一个文件都不动）。
`update_line` **不看 `idempotent_key`**（这正是旧路静默失效的原因）。其余保证原样继承：先验后写
（路径不逃逸 + 讲义哈希 + 门禁最终记录）、全或无、写入失败按备份回滚、`--git-add` 只加点名文件。

### 15.3 落地清单

| 面 | 内容 |
|---|---|
| 工具 | `publish_batch.py`：新增 `OPS` / `REPAIR_OPS` 常量、`check_record` 三件必填与单行校验、`apply_target` 三态 + "锚点必须存活"护栏；**四种旧 op 一行未改** |
| 单测 | `test_publish_batch.UpdateLineTests` **13 项**：改对 / 重复回修零变化 / 带 key 不误跳过 / `expect` 不符中止且零写入 / 子串 `expect` 被拒 / anchor 不存活被拒 / 缺字段 / 多行 / `expect==text` / 与首次发布同文件混跑 / 跨文件全或无 / dry-run 不写盘 / 门禁未通过照样拒发 |
| 自检 | `skill_selfcheck` ㉔ 新增 6 项（回修正向 / 重复回修零变化 / 4 条负向 / **工具与文档同源**：手册 §22 与 SKILL 必须都写明第五种 op，仍写「op 四种」即 FAIL）；**115 → 122 项** |
| 端到端 | `e2e_scripted_run.py` 加 5 步（回修 dry-run / 回修落盘 / 重复回修零变化 / `expect` 抄成子串的 CLI 负向 / 字节不变的内容断言）；**29 → 34 步** |
| 文档 | 操作手册新增 **§22.1**（缺口 → 旧 op 对照表 → 语义 → 与 `replace_anchor` 的差异）；SKILL 收尾段、第一册执行协议各加一句"回修走 `update_line`，**不许另写一次性脚本**"；§6.6 工具登记加一行；血证 **H29**；README 工具表与版本数字（H1~H28 → H1~H29、自检 113 → 122、单测 143 → 165） |

### 15.4 本轮验证

```text
skill_selfcheck      检查项 122，失败 0 → PASS ✅（+7：2 正向 + 4 负向 + 文档同源）
v2_selfcheck         PASS (6 contract entries)
单测                 Ran 165 tests OK（+13：UpdateLineTests）
e2e                  34 步期望全中（含回修 4 步 + 1 条内容断言）
A/B 回归             HEAD 副本 vs 当前工作树：gate_all 53 份文件**逐字段差异 0**；
                     ragent 批次1/3/33/40/43/47/48/49 + 6 份仓内样例 = 14 份单份复核
                     rc / 总判定 / [FAIL] 行**逐字一致**（本轮只动发布器与文档，闸门调用链零改动）
```

### 15.5 边界（本轮**没有**做的事）

- **没有**把发布器变成通用编辑器：一次只改一行、必须给出整行原文、anchor 必须存活——想做块级改动请用旧 op。
- **没有**改 `replace_anchor` 的 `expect` 语义（沿用子串）——旧记录的含义不能被静默改变。
- **没有**触碰 `GATE_VERSION` / SSOT：`update_line` 是发布工具行为，判据文本一个字未改，所以**不递增版本**、
  存量批次判定不受影响（A/B 已证）。
- 批次49 自身的派生视图回修仍由实现模型的一次性脚本完成（**该脚本内容本轮未复核**）；本轮交付的是
  "下次不必再写一次性脚本"的通道。批次49 的 `b49_batch.json` 是旧形态（`batch_build` 会拒绝，
  需按 §11.3 重派生）这条已知限制不变。
